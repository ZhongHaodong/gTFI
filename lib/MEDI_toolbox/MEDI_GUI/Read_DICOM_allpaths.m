function [iField,voxel_size,matrix_size,CF,delta_TE,TE,B0_dir]=Read_DICOM()
    
    DicomFolder = uigetdir(); 
...
%                 end
        ...
        case 'GE MEDICAL SYSTEMS'
            ...
            for i = 1:length(filelist)
                ...
                if mod(info.InstanceNumber,3)==2
                    iReal(:,:,slice,info.EchoNumber)  = single(dicomread(filelist{i})');%magnitude
                elseif mod(info.InstanceNumber,3)==0
                    iImag(:,:,slice,info.EchoNumber)  = single(dicomread(filelist{i})');%magnitude
                end
            end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%MOD
       ...
       save([DicomFolder,'\','RawField.mat'], 'iField','voxel_size','matrix_size','CF','delta_TE','TE','B0_dir');
