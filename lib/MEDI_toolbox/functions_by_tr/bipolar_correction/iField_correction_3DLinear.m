%校正奇偶回波线形相位差
%在图像域对相位差3D线形拟合
function iField_correction = iField_correction_3Dlinear(iField,voxel_size,mask)
iField_correction = iField;
matrix_size = size(squeeze(iField(:,:,:,1)));
mag = sqrt(sum(abs(iField).^2,4));
ErrorMap =2*angle(iField(:,:,:,2)) - (angle(iField(:,:,:,1)) + angle(iField(:,:,:,3)));

%Quick and dirty unwrapping
ErrorMap =unwrapping_gc(ErrorMap,mag,voxel_size);%Haodong
%ErrorMap = unwrapPhase(mag,ErrorMap,matrix_size);% original
...
for i=1:matrix_size(3)
    pha_filter2(:,:,i) = filter2(kernel_filter2,ErrorMap(:,:,i));
end
mask_filter2 = abs(ErrorMap-pha_filter2) < 0.2;
