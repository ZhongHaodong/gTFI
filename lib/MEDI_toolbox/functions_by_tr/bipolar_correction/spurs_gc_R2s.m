...
function [wwater wfat wfreq R2s wunwph_uf unwphw N_std ] = spurs_gc_R2s(iField,TE,CF,voxel_size,SUBSAMPLE,dfat)
if nargin<5
    SUBSAMPLE = 1;
end
...
%         wfreq = -wfreq;
    ...
    [wwater wfat wfreq R2s] = fit_IDEAL_R2((iField(:,:,:,:)), TE, CF, (wunwph_uf - 4*pi)/(2*pi*delta_TE),R2s,5);
