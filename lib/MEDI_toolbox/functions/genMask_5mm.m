...
function Mask = genMask_5mm(iField, voxel_size)
    matrix_size = size(iField(:,:,:,1));
    iMag = sqrt(sum(abs(iField).^2,4));
    m = iMag>(0.05*max(iMag(:)));           % simple threshold
    m1 = SMV(m,matrix_size, voxel_size,5)>0.999;   % erode the boundary by 10mm
    l = bwlabeln(m1,6);                       % find the biggest contiguous region
    Mask = (l==mode(l(l~=0)));
    Mask = SMV(Mask, matrix_size, voxel_size, 4)>0.001; % restore the enrosion by 4mm
end
