    ...
    for i = 1:length(filelist)
       try
        info = dicominfo([DicomFolder '/' filelist(i).name]);
            ...
            fprintf('Read data Error!! No. %d\n',i);            
            real_p(:,:,slice+1,info.AcquisitionNumber)  = real_p(:,:,slice,info.AcquisitionNumber);
            slice =slice+1;
           continue;
       end
        ...
        real_p(:,:,slice,info.AcquisitionNumber)  = single(dicomread([DicomFolder '/' filelist(i).name]))'*info.RescaleSlope+info.RescaleIntercept;
