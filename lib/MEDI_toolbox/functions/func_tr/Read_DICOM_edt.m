    ...
    switch manufacturer
        ...
        case 'SIEMENS'%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%NEED MODIFICATION%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            for i = 1:length(filelist)
                ...
                if info.EchoNumbers>NumEcho
                    NumEcho = info.EchoNumbers;
                end
                found = 0;
            ...
            for i = 1:length(filelist)
                ...
                if TE(info.EchoNumbers)==0
                    TE(info.EchoNumbers)=info.EchoTime*1e-3;
                end
                ...
                for c = 1:NumChannel 
                    ...
                    if strcmp(chaname, channelname{c})
                        channel = c;
                    end
                end
                %channel = 1;
                ...
                if (info.ImageType(18)=='P')|(info.ImageType(18)=='p')
                    iPhase(:,:,slice,info.EchoNumbers, channel)  = (single(dicomread(filelist{i})')*info.RescaleSlope+info.RescaleIntercept)/single(info.LargestImagePixelValue)*pi;%phase
                elseif (info.ImageType(18)=='M')|(info.ImageType(18)=='m')
                    iMag(:,:,slice,info.EchoNumbers, channel)  = single(dicomread(filelist{i})');%magnitude
                end
            end
